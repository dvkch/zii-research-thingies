class AddTwitterTweetsIndicies < ActiveRecord::Migration[6.1]
  def up
    ...
    execute "CREATE INDEX twitter_tweets_content_search ON twitter_tweets USING gin( to_tsvector('simple', immutable_unaccent(coalesce(twitter_tweets.content::text, ''))) );"
  end
